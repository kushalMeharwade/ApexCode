...
public class RevisePlanTool : CustomAIFunction
{
    private readonly IPlanService _planService;
    private readonly IServiceProvider _serviceProvider;

    public RevisePlanTool(IPlanService planService, IServiceProvider serviceProvider) : base(
        name: "revise_plan",
        description: "Revises the currently proposed plan based on user feedback.",
        jsonSchemaString: """
        {
            "type": "object",
            "properties": {
                "title": {
                    "type": "string",
                    "description": "Short imperative title for the revised plan."
                },
                "summary": {
                    "type": "string",
    ...
    protected override async Task<object?> InvokeCoreImplAsync(IReadOnlyDictionary<string, object?> arguments, CancellationToken cancellationToken)
    {
        ...
        var existingPlan = await _planService.GetActivePlanAsync(sessionId, cancellationToken);
        if (existingPlan == null) return "Error: No active plan found to revise.";

        var plan = new AiAssistant.Core.Models.Plan
        {
            Id       = existingPlan.Id,
            Revision = existingPlan.Revision + 1,
            Title    = arguments.TryGetValue("title", out var t) ? t?.ToString() ?? existingPlan.Title : existingPlan.Title,
        ...
        await _planService.RevisePlanAsync(sessionId, plan, cancellationToken);

        return "Plan revised successfully. The UI will now display the revision. Stop calling tools.";
    }
}
